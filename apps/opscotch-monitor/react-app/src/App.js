...
export default function App() {
  ...
  const socketRef = useRef(null); const retryRef = useRef(null); const ticketAbortRef = useRef(null); const metricSearchRef = useRef(null); const pendingMetricsRef = useRef([]); const metricFlushTimerRef = useRef(null); const generationRef = useRef(0);
  ...
  const matchingMetricEntries = browserResultsReady ? metricEntries.filter(([name, observations]) => (!debouncedMetricQuery || name.toLocaleLowerCase().includes(debouncedMetricQuery.toLocaleLowerCase())) && catalogue.matches(name, '', fieldCriteria) && (!criteria.some(criterion => criterion.operator === 'nonZero') || Number(observations.at(-1)?.value) !== 0) && matchesValueCriteria(observations.at(-1)?.value, valueCriteria)) : [];
  ...
  const addCriterion = (field, value, key, operator = 'equals') => { if (!field || !key) return; setCriteria(current => current.some(criterion => criterion.field === field && criterion.key === key && criterion.operator === operator) ? current : [...current, { field, value, key, operator }]); setMetricPage(0); };
