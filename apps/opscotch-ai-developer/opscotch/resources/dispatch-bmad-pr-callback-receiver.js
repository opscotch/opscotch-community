doc
  .description("Acknowledge CLI sidecar PR callback and process it asynchronously")
  .asUserErrors()
  .inSchema({
    type: "object",
  ...
  .dataSchema({
    ...
    properties: {
      cliSidecarDeveloperCallbackToken: { type: "string", description: "Expected callback auth token (shared with develop)" }
    }
  })
  .run(() => {
    ...
    function emitErrorMetric(errorCode) {
      var metadata = { error: "true", operation: "pr_develop", stage: "callback", error_code: errorCode, request_id: String(requestId || "") };
      context.sendMetric(context.getTimestamp(), "opscotch_ai_developer.errors", 1.0, metadata);
      context.sendMetric(context.getTimestamp(), "opscotch_ai_developer.pr_develop.errors", 1.0, metadata);
    }
    ...
    if (!expectedToken || suppliedToken !== expectedToken) {
      ...
      logDiagnostic("callback-unauthorized", { request_id: requestId, operation: "pr" });
      return;
    }
    ...
    context.sendToStepAndForget("dispatch-bmad-pr-callback-worker", JSON.stringify(body));

    logDiagnostic("pr-callback queued", {
      request_id: requestId,
      status: body.status ?? "",
