...
export default function App() {
  ...
  useEffect(() => {
    stopLive(); if (config.activeConnector !== 'metricsStore' || !config.metricsStore.apiUrl || !config.metricsStore.authorization) return undefined;
    ...
    const connect = async () => { const generation = generationRef.current; setStatus({ state: attempt ? 'reconnecting' : 'connecting', message: attempt ? 'Requesting a fresh Metrics Store ticket…' : 'Requesting a Metrics Store ticket…' }); let socket;
      ...
      try { const ticketUrl = await requestMetricsStoreTicket(config.metricsStore.apiUrl, config.metricsStore.authorization); if (cancelled || generation !== generationRef.current) return; socket = new WebSocket(ticketUrl); } catch (error) { schedule(error.message); return; } socketRef.current = socket;
